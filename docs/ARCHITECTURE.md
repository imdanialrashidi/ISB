...
- Main modules:
  - `src/pages/*.astro` — routes: `/` (index), `/about`, `/services` (consolidated service catalog), `/services/[id]` (detail pages for the 11 services flagged `detailPage: true` in `services.json`), `/projects`, `/contact`, `/blog`, `/blog/[slug]`, `404`.
  ...
  - `src/content/*.json` — the single source of content truth: `company.json`, `contacts.json`, `services.json` (16 services, 6 categories; 11 flagged `detailPage` with intro copy, related project ids and certificate references), `projects.json` (18 projects, `{id, title, client}`), `certifications.json`; plus `blog/intro-post.md` (demo post, kept `draft: true` so it never renders or indexes). A raw DOCX paragraph dump is written to `.artifacts/extracted.raw.json` (gitignored) by the extractor; `src/content/extracted.raw.json` was removed.
  - `src/lib/` — typed helpers: `types.ts` (content interfaces mirroring the JSON shape), `locale.ts` (Persian digits, fa date formatting, phone display normalization, Jalaali→Gregorian ISO conversion), `seo.ts` (SITE_URL, canonical trailing-slash convention, title rule, OG/JSON-LD builders: Organization/WebSite/BreadcrumbList/BlogPosting), `whatsapp.ts` (message/`wa.me` URL builders).
...
- External services: Cloudflare Workers (static assets + minimal Astro runtime; `wrangler.jsonc`; live deployment UNVERIFIED from the repository); Google Maps embed/links on `/contact` (embed URL lives in `contacts.json.addresses[].embedUrl`); `wa.me` and `tel:` deep links; `@astrojs/sitemap` generates `sitemap-index.xml` at build (filtered to canonical indexable URLs; `/blog/*` excluded while it carries only demo content).
